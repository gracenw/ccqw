...
from models import BertForSequenceClassification, EncoderDecoderModel, BertForQuestionAnswering
...
def scratch_sdpa_masked_unbatched(query, key, value, mask, num_attention_heads):
...
def scratch_sdpa_masked_batched(query, key, value, mask, num_attention_heads):
...
def dump_bert_parallel(type, save, verbose, dropout, home, num_accel):
    home += f'/bert_{type}'
    if save:
        if not exists(home):
            makedirs(home)
    ...
    if type == 'base':
        ...
        inputs = tokenizer(
            ...
            'I live in a 1-bedroom apartment with my cat, Molly. The apartment is in the Upper East Side neighborhood of Manhattan.', 
            # padding=True, 
            # truncation=True, 
            return_tensors="pt"
        )
        ...
        print('please pass an acceptable model type: base, large, or base2base')
        exit()    
    ...
    if type == 'base2base':
        ...
        num_attention_heads = model.config.encoder.num_attention_heads
    else:
    ...
    for l in range(num_hidden_layers):
        ...
        attn_in = activations[f"{prefix}.layer.{l}.attention.self"]["input"][0]\
        ...
        sw_attn_o, sw_scores_o = scratch_sdpa_masked_batched(sw_query_o, sw_key_o, sw_value_o, inputs["attention_mask"], num_attention_heads)

        ## first output network
        # dense_in = activations[f"{prefix}.layer.{l}.attention.output.dense"]["input"][0]
        ...
        gelu_in = activations[f"{prefix}.layer.{l}.intermediate.intermediate_act_fn"]["input"][0]
        sw_gelu_in = torch.einsum("jk,lk->jl", layernorm1_o, ffn1_w) + ffn1_b
        ffn1_o_gelu = activations[f"{prefix}.layer.{l}.intermediate.intermediate_act_fn"]["output"][0]
    ...
    if type == 'base2base':
        for l in range(num_attention_heads):
            ...
            slf_sw_attn_o, slf_sw_scores_o = scratch_sdpa_masked_batched(slf_query_o, slf_key_o, slf_value_o, inputs["attention_mask"], num_attention_heads)
            ...
            crs_sw_attn_o, crs_sw_scores_o = scratch_sdpa_masked_batched(crs_query_o, crs_key_o, crs_value_o, inputs["attention_mask"], num_attention_heads)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--type', help='model type: base, large, or base2base', type=str, required=True)
