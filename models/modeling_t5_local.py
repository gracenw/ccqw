...
from transformers import PreTrainedModel, logging, T5Config, GenerationMixin
...
class ScratchSDPAMasked(nn.Module):
    ...
    def forward(
        ...
        if len(mask.shape) < 4:
            mask = mask[:, None, None, :]
        extend_mask = mask.expand(batch_size, 1, seq_length, target_len).repeat(1, self.num_attention_heads, 1, 1)
...
class T5Attention(nn.Module):
    def __init__(
        ...
        self.sdpa_masked = ScratchSDPAMasked(
            self.n_heads, 
    ...
    def forward(
        ...
        #     key = self.k(current_states)
        #     value = self.v(current_states)
        ...
        if past_key_value is not None:
            ...
            if key_value_states is not None:
                if is_updated:
                    # print('1')
                    key = past_key_value.cross_attention_cache.key_cache[self.layer_idx]
                    value = past_key_value.cross_attention_cache.value_cache[self.layer_idx]
                else:
                    # print('2')
                    key, value = past_key_value.cross_attention_cache.update(
                ...
                # print('3')
                key, value = past_key_value.self_attention_cache.update(
        ...
        attn_output = self.sdpa_masked(query, key, value, mask, self.layer_idx)
